...
int main() {
  ...
  std::string database_path =
      std::string(home) + "/Programming/Databases/book_database.db";

  sqlite3 *db;
  int rc = sqlite3_open(database_path.c_str(), &db);
  if (rc != SQLITE_OK) {
    std::cerr << "Error opening SQLite3 database: " << sqlite3_errmsg(db)
              << std::endl
              << std::endl;
    sqlite3_close(db);
    return 1;
...
int add_to_database(sqlite3 *db, int rc) {
  ...
  const char *sql = "INSERT INTO books (title, first_name, last_name, rating, "
                    "\"read?\") VALUES (?, ?, ?, ?, ?);";
  sqlite3_stmt *stmt;
...
void search_database(sqlite3 *db, int rc, int criteria) {
  ...
  const std::string sql =
      "SELECT * FROM books WHERE " + search_column + " LIKE ? ORDER BY title;";
  sqlite3_stmt *stmt;
...
void app_title() {
  ...
  std::cout << "║║║│ ││││├┴┐├┤ └┬┘  ║  │├┴┐├┬┘├─┤├┬┘└┬┘" << std::endl;
  std::cout << "╩ ╩└─┘┘└┘┴ ┴└─┘ ┴   ╩═╝┴└─┘┴└─┴ ┴┴└─ ┴" << std::endl;
  std::cout << "by, Cristian Gutierrez" << std::endl;
  std::cout << std::endl;
}
